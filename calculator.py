...
def calculator():
    try:
        list_of_nums = microphone.voice.split()
        ...
        if oper == "+" or 'слож' in oper:
            ...
            if num_2 != 0:
                ...
                microphone.speak("Делить на ноль невозможно")
        elif "степен" in oper:
            ans = num_1 ** num_2
        microphone.speak("{0} {1} {2} = {3}".format(list_of_nums[-3], list_of_nums[-2], list_of_nums[-1], ans))
    except:
        microphone.speak("Скажите, например: Сколько будет 5+5?")